...
public class graphicWeeks extends JPanel {
 ...
 public class listenerClass implements MouseListener {
  ...
  private boolean weekEntryFound(int i) throws IOException, ClassNotFoundException{
    ...
    in.close(); 
    fileIn.close();
    for (int n = 0; n < entriesArray.size(); n++) {
      JournalEntry entry = entriesArray.get(n);
      if (entry.lifeWeek == i) {
        return true;
      }
    }
  ...
  //private JournalEntry weekEntry() {
  //}
